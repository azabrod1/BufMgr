...
BufMgr::BufMgr( int bufSize )
{
    ...
    totalCall = totalHit = numDirtyPageWrites = 0L;
    cout << "TOP HERE:  " << frames[0] << endl;


	cout << "original" << frames << endl;
...
Status BufMgr::PinPage(PageID pid, Page*& page, bool isEmpty)
{
	...
	//Replacer* policyMgr2  = new LRU_Queue(numFrames, frames, &map);
...
{
	Frame* frame;
    if(policyMgr->getUnpinnedPages() != numFrames) return OK; //FAIL; //This means there are some pages that are pinned
